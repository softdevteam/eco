...
class GraphDisplay(Display):
    ...
    def toggle_ast(self):
        self.layout.send("toggle_ast")

    def toggle_whitespace(self):
        self.layout.send("toggle_ws")

    def layout_back(self):
        self.layout.send("refresh", -1)
    ...
    def notifymousepos(self, pos):
        ...
        if node:
            if node.name in self.layout.links:
                ...
                text = "No info"
            self.setstatusbar(shortlabel(text)) # XXX
    ...
    def add_status_text(self, text, node, attr):
        version = self.layout.pgviewer.version
        s = [attr.upper(), ": "]
        try:
            r = node.log[(attr, version)]
            if r is None:
                return
            s.append(self.format_info(r))
        except KeyError:
            try:
                s.append(self.format_info(node.__getattribute__(attr)))
                s.append("(current)")
    ...
    def process_MouseMotion(self, event):
        ...
        if self.dragging:
            ...
            if event.buttons[2]:   # left mouse button
