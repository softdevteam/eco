import py
from cflexer.lexer import *
from cflexer.regex import *
from cflexer import deterministic
...
class TestDirectLexer(object):
    ...
    def test_errors(self):
        py.test.skip("incremental lexer can't return errors")
        rexs = [StringExpression("if"), StringExpression("else"),
                StringExpression("while"), StringExpression(":"),
                StringExpression(" ")]
        names = ["IF", "ELSE", "WHILE", "COLON", "WHITE"]
    ...
    def test_lookaheads(self):
        rexs = [StringExpression("aaa"), StringExpression("a"),
                StringExpression("b")]
        names = ["aaa", "a", "b"]
        l = self.get_lexer(rexs, names)
        s = "baabaaaa"
        ...
        tok = runner.find_next_token()
        assert tok.name == "b"
        assert tok.lookahead == 0
        tok = runner.find_next_token()
        assert tok.name == "a"
        assert tok.lookahead == 2
        tok = runner.find_next_token()
        assert tok.name == "a"
        assert tok.lookahead == 1
        tok = runner.find_next_token()
        assert tok.name == "b"
        assert tok.lookahead == 0
    ...
    def test_lookaheads2(self):
        rexs = [StringExpression("abc"), StringExpression("abcdefg"),
                StringExpression("b")]
        names = ["abc", "abcdefg", "b"]
        l = self.get_lexer(rexs, names)
        s = "abcdef"
        ...
        tok = runner.find_next_token()
        assert tok.name == "abc"
        assert tok.lookahead == 3

    def test_lookaheads3(self):
        rexs = [StringExpression("class"), KleeneClosure(RangeExpression("a","z")),
                StringExpression(" ")]
        names = ["class", "name", "space"]
        l = self.get_lexer(rexs, names)
        s = "class test class"
        runner = l.get_runner(s, eof=True)
        tokens = []
        tok = runner.find_next_token()
        assert tok.name == "class"
        assert tok.lookahead == 1
        tok = runner.find_next_token()
        assert tok.name == "space"
        assert tok.lookahead == 0
        tok = runner.find_next_token()
    ...
    def test_priority(self):
        rexs = []
        rexs.append(StringExpression("if"))
        rexs.append(AddExpression(RangeExpression("a", "z"), KleeneClosure(RangeExpression("a","z"))))
        names = ["if", "ID",]
        l = self.get_lexer(rexs, names)
        s = "if"
        runner = l.get_runner(s, eof=True)
