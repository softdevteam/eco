...
def gumtree_diff3(tree_base, tree_derived_1, tree_derived_2):
    ...
    # It might seem sensible to invoke Gumtree a 3rd time to generate a list of matches between
    # derived 1 and derived 2, as this would allow us to detect matches between nodes inserted by
    # derived 1 and derived 2, so that duplicate insert operations can be detected
    # Nice in theory, but in practice you end up using the matches but *ignoring* the operations
    # generated. Take Eco parse trees of the following examples:
    # BASE: `[a, b, c, d]`
    # DERIVED 1: `[a, b, x, y, c, d]`
    # DERIVED 2: `[a, b, c, z, w, d]`
    # Gumtree generates the following operations:
    # BASE -> DERIVED 1: insert `x, y` between `b` and `c`
    # BASE -> DERIVED 2: insert `z, w` between `c` and `d`
    # DERIVED 1 -> DERIVED 2: match and update `x, y, c` to `c, z, w`
    # The result is that the update operations generated by derived 1 -> derived 2 get ignored,
    # while the match between `y` and `z` causes the merged insert operations to be:
    # insert `x, (y/z)` followed by insert `(y/z), w`; the same node is inserted twice
    # resulting in an incorrect merge.
    # Conclusion: nice idea in theory but doesn't work in practice.
